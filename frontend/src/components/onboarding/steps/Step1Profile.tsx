...
const ROLES     = ['Solo Founder', 'Engineering Manager', 'Product Manager', 'Designer', 'Marketing Manager', 'Operations Lead', 'Other']
const TEAM_SIZES = ['Just me', '2–10', '11–50', '51–200', '200+']
const USE_CASES  = ['Engineering', 'Product', 'Design', 'Marketing', 'Operations', 'Cross-functional']
...
export function Step1Profile({ send, context }: Step1Props) {
  ...
  return (
    <div className="bg-white border border-rust-200 rounded-xl p-8 shadow-card">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-rust-900 tracking-tight">Tell us about yourself</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-rust-700">What's your role?</label>
          <select
            {...register('role')}
            className="w-full rounded-lg border border-rust-200 px-3 py-2.5 text-sm text-rust-900 bg-white focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent hover:border-rust-300 transition-colors"
          >
        ...
        {/* Team size */}
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-medium text-rust-700">How big is your team?</label>
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-5">
            {TEAM_SIZES.map((size) => (
              <button
                ...
                className={cn(
                  'border rounded-lg px-2 py-2 text-center text-sm font-medium transition-all duration-150',
                  selectedTeamSize === size
                    ? 'border-brand-500 bg-brand-50 text-brand-700'
                    : 'border-rust-200 text-rust-600 hover:border-rust-300 hover:bg-rust-50'
                )}
              >
                {size}
              </button>
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {USE_CASES.map((useCase) => (
              <button
                ...
                className={cn(
                  'border rounded-lg px-3 py-2.5 text-center text-sm font-medium transition-all duration-150',
        ...
        <Button type="submit" size="lg" className="w-full mt-2 bg-brand-600 hover:bg-brand-700">
          Continue
        </Button>
      </form>
    </div>
